...
import com.example.photoview.Util.Companion.checkZoomLevels
import com.example.photoview.Util.Companion.hasDrawable
import com.example.photoview.Util.Companion.isSupportedScaleType
...
class PhotoViewAttacher// Wait for the confirmed onDoubleTap() instead// Can sometimes happen when getX() and getY() is called// Check to see if the user tapped on the photo// forward long click listener
    ...
    private val onGestureListener: OnGestureListener = object : OnGestureListener {
        override fun onDrag(dx: Float, dy: Float) {
            if (mScaleDragDetector == null) return
