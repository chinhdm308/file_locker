...
import com.base.basemvvmcleanarchitecture.component.dotview.DotView
import com.base.basemvvmcleanarchitecture.component.dotview.model.Dot
...
class PatternLockView @JvmOverloads constructor(
    ...
    private var pathWidth: Int = resources.getDimensionPixelSize(R.dimen.pattern_lock_path_width)
    private var attrDotColor = Color.WHITE
    private var attrLineColor = Color.WHITE
    private var attrNormalStateColor = Color.WHITE
    private var attrWrongStateColor = Color.RED
    private var attrCorrectStateColor = Color.WHITE
    ...
    init {
        context.theme.obtainStyledAttributes(
            ...
            try {
                attrIsDotAnimate = getBoolean(R.styleable.PatternLockView_isAnimate, true)
                attrDotColor = getColor(R.styleable.PatternLockView_dotColor, Color.WHITE)
                attrLineColor = getColor(R.styleable.PatternLockView_lineColor, Color.WHITE)
                attrNormalStateColor =
                    getColor(R.styleable.PatternLockView_normalStateColor, Color.WHITE)
                attrCorrectStateColor =
                    getColor(R.styleable.PatternLockView_correctStateColor, Color.WHITE)
                attrWrongStateColor =
                    getColor(R.styleable.PatternLockView_wrongStateColor, Color.RED)
                pathWidth = getDimension(
                    R.styleable.PatternLockView_pathWidth,
                    resources.getDimensionPixelSize(R.dimen.pattern_lock_path_width).toFloat()
                ).toInt()
            } finally {
                recycle()
            }
        ...
        paint.strokeWidth = pathWidth.toFloat()
            ...
            when (motionEvent.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    if (markedDotList.size != 0 && markedDotList.size >= minCount) {
                        ...
                        state = PatternViewState.Error(attrWrongStateColor, attrWrongStateColor)
                        onChangeStateListener?.invoke(state)
                    }
    ...
    private fun createColumn(view: LinearLayout, nodeKey: String) {
        ...
        view.apply {
            addView(
                DotView(context = context, attrs = attrs, defStyleAttr = defStyleAttr).apply {
    ...
    fun getPassword(stageState: PatternViewStageState): String? = stagePasswords[stageState]
